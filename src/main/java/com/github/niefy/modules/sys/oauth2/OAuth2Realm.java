...
public class OAuth2Realm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        //token失效
        if (tokenEntity == null || tokenEntity.getExpireTime().getTime() != -1 && tokenEntity.getExpireTime().getTime() < System.currentTimeMillis()) {
            throw new IncorrectCredentialsException("token失效，请重新登录");
        }
        if (tokenEntity.getExpireTime().getTime() == -1 && IPUtils.isNotLocalRequest()) {
            throw new IncorrectCredentialsException("账号已过期,请联系管理员");
        }
