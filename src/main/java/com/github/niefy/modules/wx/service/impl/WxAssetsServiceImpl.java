...
public class WxAssetsServiceImpl implements WxAssetsService {
    ...
    public WxMpMaterialCountResult materialCount(String appid) throws WxErrorException {
        ...
        Long countDraft = wxMpService.getDraftService().countDraft();
        WxMpMaterialCountResult materialCount = wxMpService.getMaterialService().materialCount();
        materialCount.setNewsCount(countDraft.intValue());
    ...
    public WxMpMaterialNews materialNewsInfo(String appid, String mediaId) throws WxErrorException {
        ...
        List<WxMpNewsArticle> articles = draft.getNewsItem().stream().map(WxAssetsService::transform)
                .collect(Collectors.toList());
        WxMpMaterialNews news = new WxMpMaterialNews();
        news.setArticles(articles);
        news.setCreateTime(DateUtil.date());
        news.setUpdateTime(DateUtil.date());
        return news;
    }
    ...
    public WxMpMaterialNewsBatchGetResult materialNewsBatchGet(String appid, int page) throws WxErrorException {
        log.info("从API获取图文素材列表,page={}", page);
        wxMpService.switchoverTo(appid);
        final int pageSize = PageSizeConstant.PAGE_SIZE_SMALL;
        int offset = (page - 1) * pageSize;
        WxMpDraftList wxMpDraftList = wxMpService.getDraftService().listDraft(offset, pageSize);
        WxMpMaterialNewsBatchGetResult result = new WxMpMaterialNewsBatchGetResult();
        result.setItemCount(wxMpDraftList.getItemCount());
        result.setTotalCount(wxMpDraftList.getTotalCount());
        result.setItems(wxMpDraftList.getItems().stream().map(item -> {
            WxMpMaterialNewsBatchGetResult.WxMaterialNewsBatchGetNewsItem newsItem =
                    new WxMpMaterialNewsBatchGetResult.WxMaterialNewsBatchGetNewsItem();

            List<WxMpNewsArticle> articles = item.getContent().getNewsItem().stream().map(WxAssetsService::transform).collect(Collectors.toList());
            WxMpMaterialNews content = new WxMpMaterialNews();
            content.setArticles(articles);
            content.setCreateTime(DateUtil.date(item.getUpdateTime()));
            content.setUpdateTime(DateUtil.date(item.getUpdateTime()));

            newsItem.setMediaId(item.getMediaId());
            newsItem.setUpdateTime(DateUtil.date(item.getUpdateTime()));
    ...
    public boolean materialDelete(String appid, String mediaId) throws WxErrorException {
        ...
        wxMpService.switchoverTo(appid);
        return wxMpService.getDraftService().delDraft(mediaId);
    }
}
