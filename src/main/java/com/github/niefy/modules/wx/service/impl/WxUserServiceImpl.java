...
public class WxUserServiceImpl extends ServiceImpl<WxUserMapper, WxUser> implements WxUserService {
    ...
    public WxUser refreshUserInfo(String openid, String appid) {
        try {
            ...
            WxUser user = new WxUser(openid, appid);
    ...
    public void syncWxUsers(String appid) {
        ...
        WxAccountService wxAccountService = SpringContextUtils.getBean("wxAccountService", WxAccountService.class);
        WxAccount wxAccount = wxAccountService.getById(appid);
        boolean isNotExpire = WxMpHtmlPageApiUtil.cookieIsNotExpire(wxAccount);
        if (isNotExpire) {
            String cookies = wxAccount.getHtmlPageCookies();
            syncWxUsersByCookies(cookies);
        } else {
            syncWxUsersByOpenApi(appid);
        }
    }
     ...
     * 同步用户列表,公众号一次拉取调用最多拉取10000个关注者的OpenID，可以通过传入nextOpenid参数多次拉取
     */
    private void syncWxUsersByCookies(String cookies) {
        logger.info("根据token，同步公众号粉丝列表：功能还未完成");
    }
     ...
     * 同步用户列表,公众号一次拉取调用最多拉取10000个关注者的OpenID，可以通过传入nextOpenid参数多次拉取
     */
    private void syncWxUsersByOpenApi(String appid) {
